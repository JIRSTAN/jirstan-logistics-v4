...
const defaultState = {
    ...
    vehicles: [ {id: 1, type: 'semi', model: 'Scania S770 V8', driverId: 1, loc: 'Praha', job: null, queue: [], progress: 0, cond: 100, fuel: 100, spd: 1.6, upgrades: [], trailer: null} ],
    drivers: [ 
    ...
    gasNetwork: { level: 0, hasDiner: false, hasShop: false }
};
...
function renderAll() {
    ...
    renderBuses(); renderChallenges(); renderBank(); renderInvestments(); updateFuelUI(); drawFuelChart(); drawMarketChart();
    renderFactions(); renderInsurance(); renderOverseas(); renderGasNetwork();
}
...
function daily() {
    ...
    state.buses.forEach(bus => {
        ...
        // Vypočítání příjem a náklady
        const revenue = Math.floor(route.dailyIncome * (1 + (state.hq.logistics_center * 0.03)));
        addMoney(revenue);
        ...
        if(bus.fuel <= 5 || bus.cond <= 10) {
            notify("AUTOBUSY", `Autobus ${bus.model} potřebuje doplnit palivo nebo servis na trase ${route.name}.`, "warning");
...
function renderGasNetwork() {
    ...
    let html = `
        ...
        <div class="grid" style="grid-template-columns: repeat(3, 1fr); gap: 25px;">
...
function renderBuses() {
    ...
    if (shopEl) {
        shopEl.innerHTML = BUS_DB.map(b => `
            <div class="card">
                <img src="${b.img}" class="card-img" onerror="this.style.display='none'">
                <div class="card-body">
                    <h3>${b.model}</h3>
...
        btn.classList.add('active');
        
        if (tabId === 'dispatch') { setTimeout(fixMapSize, 50); } else if (tabId === 'auction') { renderAuction(); }        if (tabId === 'stats') renderStats();
